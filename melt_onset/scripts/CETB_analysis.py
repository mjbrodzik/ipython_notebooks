...
def MOD_array(datadir, prefix, CETB_data, DAV, rows_cols, cal_date, Years, window, count, DAV_threshold, Tb_threshold):
	...
	# gets a dataframe with one row for each pixel, one column for each year, MOD in DOY in each cell for that pixel and year
	dates=np.empty(0)
...
def MOD_array_year(datadir, prefix, CETB_data, DAV, rows_cols, cal_date, year, window, count, DAV_threshold, Tb_threshold):
	#GET MOD FRAME FOR SIR PIX
	# full matrix of MODs for every pixel in subset - SIR
	y_s=list(range(rows_cols[0],rows_cols[1]))
	x_s=list(range(rows_cols[2],rows_cols[3]))
